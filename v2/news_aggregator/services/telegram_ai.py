"""Telegram AI client for news categorization and digest generation."""
...
class TelegramAI:
    """AI client for Telegram news processing."""
    
    def __init__(self):
        # Load Telegram AI configuration
        ...
        if not all([self.api_url, self.api_key, self.model]):
            raise APIError("Telegram AI configuration incomplete")
    ...
    @cached(ttl=3600, key_prefix="telegram_category")
    ...
    async def generate_daily_digest(self, articles_by_category: Dict[str, List], 
        ...
        """
        Generate daily news digest for Telegram (like old version).
    ...
    def _build_digest_prompt(self, articles_by_category: Dict[str, List], 
        ...
        # Add news data by category (like old version)
    ...
    async def _make_ai_request(self, prompt: str) -> str:
        """Make request to Telegram AI API with retry and timeout."""
        import asyncio
                ...
                async with get_http_client() as client:
                    ...
                    async with response:
                        ...
                        if response.status == 200:
                            ...
                            logging.error(f"Telegram AI API error {response.status}: {error_text}")
                            return "Error"
    ...
    def _create_fallback_digest(self, articles_by_category: Dict[str, List], 
                              header_text: str) -> str:
        """Create simple fallback digest (like old version)."""
        import datetime
        ...
        for category, articles in articles_by_category.items():
            ...
            # Create narrative from article headlines (like old version)
...
# Global Telegram AI instance
_telegram_ai: Optional[TelegramAI] = None


def get_telegram_ai() -> TelegramAI:
    """Get Telegram AI client instance."""
    global _telegram_ai
    
    if _telegram_ai is None:
        _telegram_ai = TelegramAI()
